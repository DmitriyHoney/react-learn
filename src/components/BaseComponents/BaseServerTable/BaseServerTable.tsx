...
import baseServerTableReducer, {
  CountActionKind,
  initialBaseServerState,
  IState,
} from "./BaseServerTableReducer.ts";
...
export default function BaseServerTable({ headers, apiKey, title }: IProps) {
  let initialState: IState = initialBaseServerState;
  const getStateFromLocalStorage = localStorage.getItem(`params_${apiKey}`);

  if (!getStateFromLocalStorage) {
    localStorage.setItem(
      `params_${apiKey}`,
      JSON.stringify(initialBaseServerState)
    );
  } else
    initialState = JSON.parse(localStorage.getItem(`params_${apiKey}`) || "{}");

  const updateLocalStorage = (state: IState) =>
    localStorage.setItem(`params_${apiKey}`, JSON.stringify(state));

  const [params, dispatch] = useReducer(baseServerTableReducer, initialState);
    ...
    <tr key={i.id}>
      {headers.map((h, idx) => {
        ...
        if (typeof value === "object") textContent = "-";
          ...
          textContent = value ? "Активен1" : "Неактивен";
